...
\begin{lstlisting}[language=TypeScript, caption={Inbox Screen — Filter Logic}]
const FILTERS = [
  { label: 'ALL',        value: 'all' },
...
// Filter events by active category chip
const filteredEvents = filter === 'all'
  ? events
  : events.filter(e => e.category === filter);

// Force re-scan: clear server-side cache + trigger fresh Gmail analysis
const handleRescan = async () => {
  await api.post('/personal-events/rescan');
  // Re-analysis happens within 5 minutes on next watcher cycle
  fetchEvents();
};
\end{lstlisting}
